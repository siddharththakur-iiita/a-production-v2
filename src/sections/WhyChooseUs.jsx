...
const WhyChooseUs = memo(() => {
  ...
  const subtleY = useTransform(scrollYProgress, [0, 1], shouldReduceMotion ? [0, 0] : [14, -14]);
  ...
  const itemVariants = useMemo(
    () => ({
      ...
      show: {
        opacity: 1,
        y: 0,
        transition: { duration: shouldReduceMotion ? 0.4 : 1.5, ease: EASE_LUXURY }
      }
    }),
    [shouldReduceMotion]
  );
        ...
        <div className="grid grid-cols-1 gap-16 lg:grid-cols-12 lg:gap-20 xl:gap-24">
          ...
          >
            ...
            <motion.div variants={itemVariants}>
              <Link
                to="/about"
                className="group inline-flex items-center justify-center rounded-full bg-neutral-900 px-10 py-4 text-[11px] font-medium uppercase tracking-[0.2em] text-white outline-none ring-1 ring-transparent transition-all duration-700 ease-[var(--ease-luxury)] hover:-translate-y-1 hover:bg-black hover:shadow-[0_20px_40px_-10px_rgba(0,0,0,0.3)] focus-visible:ring-2 focus-visible:ring-neutral-900 focus-visible:ring-offset-4 focus-visible:ring-offset-[#FCFBF9]"
                aria-label="Discover our heritage and craftsmanship"
              >
                <span>Discover Our Heritage</span>
                <ArrowRight
                  className="ml-4 h-4 w-4 transition-transform duration-700 ease-[var(--ease-luxury)] group-hover:translate-x-2"
                  strokeWidth={1.5}
                  aria-hidden="true"
                />
              </Link>
              ...
              {features.map((feature, index) => {
                ...
                return (
                  <motion.article
                    ...
                    className={`group relative flex flex-col items-start overflow-hidden rounded-[2.5rem] bg-[#FAFAFA]/40 border border-black/[0.02] p-10 md:p-14 lg:p-16 backdrop-blur-md transition-all duration-700 ease-[var(--ease-luxury)] hover:-translate-y-1.5 hover:bg-white hover:shadow-[0_20px_40px_-10px_rgba(0,0,0,0.04)] hover:border-black/[0.03] ${isEven ? 'sm:mt-24 lg:mt-32 xl:mt-40' : ''}`}
                  >
                    <div className="pointer-events-none absolute inset-0 z-0 bg-gradient-to-br from-white/60 to-transparent opacity-0 transition-opacity duration-700 ease-[var(--ease-luxury)] group-hover:opacity-100" />

                    <div className="relative z-10 mb-10 flex h-12 w-12 items-center justify-center rounded-full border border-black/[0.03] bg-white/60 transition-colors duration-700 ease-[var(--ease-luxury)] group-hover:bg-[#FDFCFB]">
                      <Icon className="h-4 w-4 text-[#8C8273]" strokeWidth={1.5} aria-hidden="true" />
                    </div>

                    <span className="relative z-10 mb-6 block text-[8px] font-medium uppercase tracking-[0.3em] text-[#8C8273] transition-colors duration-700 ease-[var(--ease-luxury)] group-hover:text-[#666666]">
                      {feature.label}
                    </span>

                    <h3
                      id={`feature-title-${feature.id}`}
                      className="relative z-10 mb-6 font-serif text-2xl font-light tracking-wide text-[#111111] md:text-3xl"
                    >
                      {feature.title}
                    </h3>
