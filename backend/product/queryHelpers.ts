...
>(
  query: PostgrestFilterBuilder<TSchema, TRow, TResult, TRelationships>,
  sort: SortParam<TColumn> | undefined,
  fallback: SortParam<TColumn>
): PostgrestFilterBuilder<TSchema, TRow, TResult, TRelationships> {
  const resolved = sort ?? fallback;
  return query.order(resolved.column, {
    ascending: (resolved.direction ?? 'asc') === 'asc',
  }) as PostgrestFilterBuilder<TSchema, TRow, TResult, TRelationships>;
}
